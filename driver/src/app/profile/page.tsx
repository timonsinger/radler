...
export default function DriverProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (driverData.stats) setStats(driverData.stats);
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (userData.user) {
        setUser((prev) => prev ? { ...prev, ...userData.user } : prev);
        const stored = getStoredUser();
        if (stored) {
          localStorage.setItem('user', JSON.stringify({ ...stored, name: userData.user.name, phone: userData.user.phone }));
        }
      }
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="bg-primary px-5 pt-12 pb-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-white">Mein Profil</h1>
        </div>
      </div>
      ...
      <div className="px-4 pt-5 space-y-4">
        ...
        {/* Fahrer-spezifisch */}
        <div className="bg-white rounded-3xl p-5 shadow-sm space-y-4">
          <h3 className="font-semibold text-gray-900">Fahrer-Profil</h3>
          ...
          <div>
            <div className="flex items-center justify-between">
              <label className="text-xs text-gray-400 font-semibold uppercase">Über mich</label>
              <span className="text-xs text-gray-300">{description.length}/200</span>
            </div>
            <textarea
              value={description}
              ...
              rows={3}
              className="w-full mt-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none"
            />
          </div>

          <div>
            <label className="text-xs text-gray-400 font-semibold uppercase">Verfügbarkeit</label>
            <input
              type="text"
              value={availability}
              onChange={(e) => setAvailability(e.target.value.substring(0, 100))}
        ...
        <button
          ...
          className={`w-full font-semibold py-4 rounded-2xl transition-colors ${
            saved ? 'bg-green-500 text-white' : 'bg-primary text-white active:bg-primary/80'
          } disabled:opacity-60`}
        >
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2">
            <Link href="/reviews" className="bg-gray-50 rounded-2xl p-3 text-center active:bg-gray-100 transition-colors block">
            ...
            <div className="bg-gray-50 rounded-2xl p-3 text-center">
              ...
              <p className="text-[10px] text-gray-400 mt-1">Verdient</p>
            </div>
          </div>
        </div>
