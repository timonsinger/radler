  ...
  useEffect(() => {
    ...
    apiFetch(`/api/rides/history?page=${page}&limit=20`)
      .then((data) => {
        ...
        if (data.total !== undefined) setTotalRides(data.total);
        if (data.totalEarnings !== undefined) setTotalEarnings(Number(data.totalEarnings));
      })
      .catch(console.error)
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="bg-primary px-5 pt-12 pb-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black text-white">Fahrtenhistorie</h1>
        </div>
      </div>
      ...
      <div className="px-4 pt-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-700">Gesamt: {totalRides} Fahrten</span>
          </div>
          <span className="text-sm font-bold text-primary">{totalEarnings.toFixed(2).replace('.', ',')} € verdient</span>
        </div>
      </div>
      ...
      <div className="px-4 space-y-3">
        {rides.length === 0 && !loading && (
          <div className="text-center py-16 bg-white rounded-3xl shadow-sm">
            ...
            <p className="text-gray-500">Noch keine abgeschlossenen Fahrten</p>
          </div>
        )}
            ...
            <div className="px-5 pb-8 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="bg-gray-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-sm font-bold truncate">{selectedRide.customer_name || '–'}</p>
                </div>
              </div>
